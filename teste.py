...
def getNews():
    ...
    articles = soup.find_all(
        'article', class_="MQsxIb xTewfe R7GTQ keNKEd j7vNaf Cc0Z5d YKEnGe EyNMab t6ttFe Fm1jeb EjqUne")
    news = [i.find_all('a', class_="ipQwMb Q7tWef")[
        0].text for i in articles[:5]]
    links = [root+i.find('a')['href'][1:] for i in articles[:5]]
    links = [requests.get("http://thelink.la/api-shorten.php?url=" +
                          link).content.decode() for link in links]
    for i in range(5):
...
while True:
    ...
    # Clicking couple of times because sometimes whatsapp web responds after two clicks
        try:
            ...
            pass
        try:
            name = browser.find_element_by_class_name(
                "_2zCDG").text  # Contact name
            message = browser.find_elements_by_class_name(
                "vW7d1")[-1]  # the message content
